...
// Find all carousel items and set up the initial active item
...
function loopCarousel() {
  ...
  // Move to the next item, or back to the first if we reach the end
...
// Set interval for automatic looping (e.g., 2 seconds)
...
document.getElementById("proceedBtn")?.addEventListener("click", async () => {
  ...
  // Check if the user is logged in by retrieving their data from localStorage
  ...
  if (user) {
    ...
    const response = await fetch(`${API}/create-order`, {
      ...
      body: JSON.stringify({
        user_id: user.user_id, // Make sure this matches the data from your backend
        event_id: 1,  // Assuming event ID is 1; update accordingly
        ticket_type: type,
        qty: qty,
      }),
    });
    ...
    if (response.ok) {
      ...
      window.location.href = `/checkout.html?order_id=${orderData.order_id}&total=${orderData.total}`;
    } else {
      // Handle errors if order creation fails
      alert("Error creating order: " + orderData.detail);
    }
  } else {
